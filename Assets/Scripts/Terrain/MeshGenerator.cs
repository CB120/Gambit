...
{
    public static MeshData GenerateTerrainMesh(float[,] heightMap, float waterThreshold, float mountainThreshold, float peakThreshold, float topThreshold)
    {
        ...
        for (int y = 0; y < tilesHeight; y++)
        {
            ...
            for (int x = 0; x < tilesWidth; x++)
            {
                ...
                bool isGroundTile = isWaterTile ? false : heightMap[x, y] < mountainThreshold;
                bool isMountainTile = isWaterTile ? false : isGroundTile ? false : heightMap[x, y] < peakThreshold;
                bool isPeakTile = isWaterTile ? false : isGroundTile ? false : isMountainTile ? false : heightMap[x, y] < topThreshold;
                bool isTopTile = isWaterTile ? false : isGroundTile ? false : isMountainTile ? false : isPeakTile ? false : true;

                int vertexIndex = vertices.Count;

                float height = (isWaterTile) ? (defaultHeight - waterDepth) : (isGroundTile) ? defaultHeight : (isMountainTile) ? (defaultHeight + 1) :  (isPeakTile) ? (defaultHeight + 2) : (isTopTile) ? (defaultHeight + 3) : defaultHeight;
                ...
                if (!isWaterTile || isEdgeTile)
                {
                    for (int i = 0; i < nswe.Length; i++)
                    {
                        ...
                        if ((!isGroundTile || isEdgeTile) || isNeighbourWaterTile)
                        {
                            float depth = height; // by default set the depth to 1

                            if (isNeighbourOutside || (!isWaterTile && isNeighbourWaterTile))
                            {
                                depth = (isNeighbourOutside) ? height : waterDepth; //if water set depth to waterDepth
                            }
                            else if(isEdgeTile && isWaterTile)
                            {
                                //ensure the water tile does not overextend
                                depth = 1 - waterDepth;
                            }
